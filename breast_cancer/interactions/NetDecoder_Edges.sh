...
OUT_DIR=~/NetDecoder_Example/breast_cancer/interactions/
...
corThreshold=0.5
top=10
geneList=p53_pathway_genes.txt

java -cp $LIB "netdecoder.NetDecoder" -SYMBOL $SYMBOL -GO $GO -out $OUT_DIR -E -control $ann1 -condition $ann2 -ncp $netpath1 -ndp $netpath2 -corThreshold $corThreshold -ratioThreshold $ratioThreshold -top $top -g $geneList -f ERnegative
